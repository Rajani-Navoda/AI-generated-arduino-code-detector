...
const int motorPin1 = 9; // PWM output pin 1 for H-bridge
const int motorPin2 = 10; // PWM output pin 2 for H-bridge
const int motorEnablePin = 8; // Enable pin for H-bridge
...
void loop() {
  ...
  int motorSpeed = map(potValue, 0, 1023, 0, 255); // Map pot value to PWM range (0-255)
...
void speedControl(int motorSpeed) {
  int direction = digitalRead(potPin) > 512 ? 1 : 0; // Determine direction based on pot position
